...
int attack_slash(t_char* character, t_enemy* enemy) {
  ...
  if (!strcmp("Warrior", character->class->name) && rand_between(1, 10) == 1) {
    printf(" ... and then valiantly tackle him down, inflicting him two times "
           "that damage !\n");
    enemy->hp -= dmg;
  } else {
    character->hp -= 2;
    printf(", but also hurt yourself in the process and lose " COLOR_RED
           "2" COLOR_CLEAR " HP !\n");
  }
  enemy->hp -= dmg;
  return CONSUME_TURN;
...
int attack_fire(t_char* character, t_enemy* enemy) {
  ...
  if (character->mp >= 2) {
    ...
    if (char_crit(&dmg, character, 0))
      printf(MESSAGE_CRITICAL_HIT);
    printf(COLOR_CLEAR " damages, costing you " COLOR_BLUE "3" COLOR_CLEAR
                       " mana.\n");
    character->mp -= 3;
    enemy->hp -= dmg;
    return CONSUME_TURN;
...
int attack_thunder(t_char* character, t_enemy* enemy) {
  ...
  if (character->mp >= 5) {
    ...
    if (char_crit(&dmg, character, 0))
      printf(MESSAGE_CRITICAL_HIT);
    printf(COLOR_CLEAR " damages to your enemy, costing you " COLOR_BLUE
                       "5" COLOR_CLEAR " mana.\n");
