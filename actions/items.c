  ...
  (void)enemy;
	...
	else if (character->potions)
	{
		...
		if (rand_between(1, 10) == 1)
			printf("You try to get a health potion out of your bag, but you accidentally break it! It's unusable!\n");
		else
		{
      int heal = 30;
			printf("You drink a potion, regaining "COLOR_GREEN "%i" COLOR_CLEAR" HP.\n",
          heal);
...
int	item_ether(t_char *character, t_enemy *enemy)
{
  (void)enemy;
	...
	else if (character->ethers)
	{
		--character->ethers;
    int mana = 12;
		printf("You use an ether, regaining "COLOR_BLUE "%i" COLOR_CLEAR" mana.\n",
        mana);
